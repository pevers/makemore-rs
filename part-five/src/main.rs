...
/// Load the data as two arrays with block size 8
fn load_data(path: &str) -> (Vec<u32>, Vec<u32>) {
    ...
    for word in words {
        ...
        let mut context = [0, 0, 0, 0, 0, 0, 0, 0];
        for curr in chars {
            let ix = stoi(curr);
            xs.extend(context);
            ys.push(ix);
...
fn train<B: AutodiffBackend>(device: B::Device) -> Model<B> {
    ...
    for i in 0..50 {
        ...
        for (xs_batch, ys_batch) in &batches {
            ...
            let input_tensor: Tensor<B, 2, Int> = input_tensor.reshape([-1, 8]);
...
pub fn inference<B: Backend>(model: &Model<B>, device: B::Device) {
    for _ in 0..5 {
        ...
        loop {
            ...
            name.push(c);
            input = [
                input[1], input[2], input[3], input[4], input[5], input[6], input[7], idx,
            ];
        }
